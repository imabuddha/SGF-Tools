...
struct ThumbnailTests {
    ...
    /// A thumbnail drawn as the extension draws it, into a bitmap `size` times `scale` pixels.
    private func draw(_ thumbnail: Thumbnail, size: CGSize, scale: CGFloat) -> Bitmap {
        let bitmap = Bitmap(width: Int((size.width * scale).rounded()), height: Int((size.height * scale).rounded()))
        bitmap.context.scaleBy(x: scale, y: scale)
        thumbnail.draw(in: bitmap.context, rect: CGRect(origin: .zero, size: size))
        return bitmap
    }
    ...
    @Test func aGameGetsItsOpeningPosition() throws {
        ...
        let pixels = draw(thumbnail, size: size, scale: 2)
        #expect(pixels.width == 512 && pixels.height == 512)
    ...
    @Test func aCollectionGetsTheBackdrop() throws {
        ...
        #expect(thumbnail.isCollection)
        let pixels = draw(thumbnail, size: size, scale: 1)
    ...
    @Test func fastEnough() throws {
        ...
        for _ in 0 ..< 20 {
            let duration = try clock.measure {
                ...
                _ = draw(thumbnail, size: CGSize(width: 512, height: 512), scale: 2)
            }
