...
extension BoardSize {
    ...
    ///
    /// Square boards follow SGF Tools 1.x exactly:
    ...
    /// - odd sizes from 13x13 up: also the four side points, midway along the corner points' lines.
    ///
    ...
    /// - the corner points sit on the 1.x corner lines of each direction (second, third, or
    ///   fourth line, as above); a direction of 3 or 4 lines has none
    /// - the center point needs an odd number of both columns and rows
    /// - the top and bottom side points need an odd number of at least 13 columns, and the left
    ///   and right side points an odd number of at least 13 rows.
    ///
    /// A 19x13 board has nine: corners on the fourth lines, the center, and all four side
    /// points. A 19x9 board has seven: corners on the fourth columns and third rows, the center,
    /// and side points at the top and bottom only. A board with a dimension of 1 or 2 has none.
...
private struct StarLines {
    ...
    /// Whether the middle line carries side points (13 or more lines, odd).
    ...
    init(lineCount: Int) {
        ...
        hasSidePoints = lineCount % 2 == 1 && lineCount >= 13
    }
}
